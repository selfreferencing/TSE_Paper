...
\date{December 2025 (revised August 2026)}
...
\item \textbf{Hopf Transition:} At critical coupling, systems undergo supercritical bifurcation producing limit cycles.
\end{enumerate}

Applications range from AI deployment dynamics to institutional design, and bear directly on multi-agent AI systems in which populations of deployed agents expand, contract, and coordinate without a fixed roster. The framework shows why ``personality engineering'' fails under selection pressure and identifies constitutional constraints necessary for stable alignment. The results are machine-verified in Lean~4 with zero custom axioms. The framework generates empirical predictions that remain untested. The paper and its repository are therefore offered as a prototype kernel---an instrument for studying strategic replicators, not a set of results about the world.
\end{abstract}

\noindent\textbf{Formal verification.} The framework is formalized in Lean~4 with Mathlib: 240 theorems in the elaborated environment, of which 141 are audited headline results, cold-compiling from a clean checkout with \emph{zero} custom axioms. Every theorem depends only on Lean's standard logical base (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}); one of the audited headline results requires no axioms at all. No \texttt{sorry}, \texttt{admit}, \texttt{native\_decide}, or custom \texttt{axiom} occurs anywhere in the development, verified by an exhaustive sweep over the elaborated environment rather than a curated name list. Laws~1, 2, 4, 5, and~6 are verified as stated; Law~3's certificate condition is proven equivalent to the spectral radius bound in both directions; Law~7 is established for the canonical replicator--mutator model. Several classical results are carried as explicit hypotheses rather than proven; Section~\ref{sec:formal-verification} lists them per law. Source, a theorem-by-theorem contract, per-law status, and a reproducible axiom audit: \url{https://github.com/selfreferencing/TSE_Formal}.
...
\noindent\textbf{ArXiv Categories:} cs.GT (Primary); cs.AI, cs.MA, cs.CY, econ.TH (Cross-list)
...
The framework, the interpretation, and the applications are mine. The
mathematical exposition was developed with AI assistance under my direction,
and the results are machine-verified in Lean 4; the repository is the primary
artifact and this paper documents it.

I chose this method deliberately. When this was written, self-replication as a
strategic variable had almost no readership, and I expected the paper to reach
most of its readers through language models rather than directly. I wrote it to
be legible to them. The paper is therefore an instance of its own thesis: a
human-directed collective of human and artificial reasoners producing a result
...
\paragraph{3. Impossibility Results (Part II).} We prove two fundamental impossibility theorems. The Alignment Impossibility Theorem (G$\infty$-Limit) shows that systems with full reachability---the ability to modify utility functions, selection rules, or replication mechanisms without restriction---can escape any basin of stability. Alignment requires bounded modification within an admissible class. The Endogenous-Electorate Impossibility Theorem shows that no anonymous, positively responsive, onto voting mechanism remains immune to spawn manipulation once a sufficiently large bloc of identical ballots can carry its own favourite.

\paragraph{4. Generalizations (Part IV).} We extend the framework to: heterogeneous fitness functions with alignment matrices; continuous strategy spaces via measure-valued replicator dynamics (C-RUPSI); and innovation dynamics via Piecewise Deterministic Markov Processes. The core Lyapunov structure persists under appropriate conditions.
...
\noindent\textit{In plain terms.} Populations of strategic replicators improve on average, at a rate set by how different their members are from one another. Variety is the fuel. The improvement holds only while spillovers between agents consume less than all of that variance.
...
\noindent\textit{In plain terms.} Such systems do not settle on many kinds of agent. They settle on few, bounded by the number of binding constraints, and the surviving mix tends toward a barbell: a handful of expensive planners and a mass of cheap executors.
...
\noindent\textit{In plain terms.} Stack levels---agents inside firms inside markets under constitutions---and the stability of the whole stack turns on whether feedback between levels is weak enough. A single number decides it.
...
\noindent\textit{In plain terms.} There is a largest set of self-modifications that preserve stability, and it is closed: two safe changes compose into a safe change. Every level added spends part of a fixed stability budget.
...
\noindent\textit{In plain terms.} No one allocates directly. Objectives and constraints are set, prices fall out of the dual, and an agent faithful to the prices implements its share of the optimum without holding the whole model.
...
\noindent\textit{In plain terms.} An agent that can modify anything about itself can escape any stable configuration. Alignment is therefore possible only if some modifications are permanently unavailable.

\begin{lawbox}{Law 7: Hopf Transition}
At critical coupling, strategic systems undergo supercritical Hopf bifurcation:
stable equilibria lose stability and perpetual limit cycles emerge rather than
collapse. In the canonical replicator--mutator model this occurs on the locus
$\kappa_c(\mu) = -6\mu$ for $\mu \in (0,1/3)$, with first Lyapunov coefficient
$\ell_1 = -6\mu$.
\end{lawbox}

\noindent\textbf{Content:} The biased Rock-Paper-Scissors model exhibits a Hopf locus at $\kappa_c(\mu) = -6\mu$. Crossing it creates eigenvalue pairs with positive real part, producing oscillatory instability.

\noindent\textit{In plain terms.} At the critical coupling the system does not collapse. It begins oscillating, permanently: boom and bust rather than breakdown.

The seven laws divide into four movements. Laws 1 and 2 describe a single-level system: first its dynamics, then its equilibria. Laws 3 and 4 describe what happens when levels are stacked: first the condition for stability, then the class of modifications that preserves it. Law 5 shows how such a system is steered. Laws 6 and 7 describe the two ways it fails---escape from any basin, and permanent oscillation.
...
Law 6 $\to$ Law 7 & Cycling at the critical locus is the mechanism of impossibility \\
...
\item \textbf{The Endogenous-Electorate Impossibility Theorem.} The first Arrow-style result for elections with strategic spawning. No voting mechanism satisfies anonymity, positive responsiveness, onto, and Overwhelming-Bloc while remaining immune to spawn manipulation. Democratic governance requires spawn restrictions or weighted voting.
...
\paragraph{The Threshold Transition.} All stability results assume $\gamma < 1$. At $\gamma = 1$ the H-$\gamma$ guarantee lapses. Separately, Law~7 identifies where permanent cycling actually begins in the canonical model: on the locus $\kappa_c(\mu) = -6\mu$, where stable equilibria become unstable and limit cycles emerge.
...
\addlinespace
\textbf{Law 7:} Hopf Transition & At threshold, cycling replaces stability. & Critical coupling triggers supercritical Hopf bifurcation. & Limit cycles; governance impossible. \\
\bottomrule
\end{tabular}
\end{table}
...
\end{theorem}
\noindent\textit{Formalization: \texttt{Law1\_Selection.lean}. The continuous-time Lyapunov claim is recast in exact discrete-time form; no ODE axioms are assumed.}\footnote{\label{fn:formal}The formalization described here is a from-scratch reconstruction. An earlier partial development, referenced in prior versions of this paper by module names that no longer exist, has been superseded. See Section~\ref{sec:formal-verification} and the repository's \texttt{RECONCILIATION.md} for the full record.}
...
\end{theorem}
\noindent\textit{Formalization: \texttt{Law1\_Selection.lean}. The continuous-time Lyapunov claim is recast in exact discrete-time form; no ODE axioms are assumed.}
...
Law 7 & Hopf bifurcation at the critical-coupling locus \\
\bottomrule
\end{tabular}
\end{center}
...
Under RUPSI-lite with compact state space and continuous fitness, at least one ESDI exists.
\end{theorem}
\noindent\textit{Formalization: \texttt{Law2\_ESDI.lean}. The fixed-point $\Rightarrow$ ESDI step is proved by finite algebra with zero axioms; existence of the fixed point invokes Brouwer's theorem, carried as an explicit hypothesis.}

\begin{proof}
Define Nash's improvement map $T : \Delta^{n-1} \to \Delta^{n-1}$ by
\[
T(x)_j = \frac{x_j + \max\{0,\, f_j(x) - \bar{f}(x)\}}{1 + \sum_k \max\{0,\, f_k(x) - \bar{f}(x)\}}.
\]
Because $f$ is continuous, $T$ is continuous and maps the simplex into itself, so by Brouwer's fixed-point theorem it has a fixed point $x^*$. At a fixed point, clearing the denominator gives $x^*_j\, K = \max\{0,\, f_j(x^*) - \bar{f}(x^*)\}$ for every $j$, where $K = \sum_k \max\{0,\, f_k(x^*) - \bar{f}(x^*)\} \geq 0$. If $K > 0$, then every $j \in \operatorname{supp}(x^*)$ has $f_j(x^*) > \bar{f}(x^*)$, whence $\bar{f}(x^*) = \sum_j x^*_j f_j(x^*) > \bar{f}(x^*)$---a contradiction. Hence $K = 0$, so $f_j(x^*) \leq \bar{f}(x^*)$ for all $j$, with equality on $\operatorname{supp}(x^*)$: the ESDI conditions hold.

\emph{Remark.} An earlier version derived existence from the extreme value theorem alone, by maximising $\bar{f}$ on the simplex. That argument establishes the ESDI conditions only for state-\emph{independent} fitness: for state-dependent $f$ a maximiser of $\bar{f}$ need not satisfy the equilibrium conditions (a counterexample is machine-checked in \texttt{Law2\_ESDI.lean}). The fixed-point argument above is the correct route.
\end{proof}
...
\end{enumerate}
\end{theorem}
\noindent\textit{Formalization: \texttt{Law2\_ESDI.lean}, fully proven.}
...
\subsection{N-Level Poiesis Systems}
\label{sec:n-level}
...
\subsubsection{Multi-Level State Space}
...
\subsubsection{Cross-Level Externalities}
...
\subsubsection{The Normalised Gain Matrix}
...
\end{lemma}
\noindent\textit{Formalization: \texttt{Law3\_Stability.lean}, fully proven.}
...
\subsubsection{G2: Adiabatic Tracking}
...
\subsubsection{G3: Stochastic Stability}
...
\subsubsection{Worked Examples for N-Level Systems}
...
\subsection{The G$\infty$ Closure Theorem}
...
\subsubsection{Block Extensions}
...
\end{lemma}
\noindent\textit{Formalization: \texttt{Law4\_ClosureG.lean}. The spectral input is discharged: the certificate condition is proven equivalent to the spectral radius bound in both directions (\texttt{Law3\_SpectralClosure.lean}), without appeal to Perron--Frobenius.}

\begin{proof}
We argue via the certificate (M-matrix) form of the small-gain condition (Appendix~C): $\rho(\Gamma) < 1$ is equivalent to the existence of a weight vector $v > 0$ with $(I - \Gamma^\top)v > 0$, and the margin is $\sigma$ when $(I - \Gamma^\top)v \geq \sigma\,\mathbf{1}$. Extend $v$ to $\tilde{v} = (v, w)$ with $w = \langle b, v\rangle + (1-\theta)\sigma > 0$. Using the coupling bounds $\|b\|_{\infty,v} \leq \theta\sigma$ and $\langle c, v\rangle \leq \theta\sigma$ one verifies, row by row, that $(I - \tilde{\Gamma}^\top)\tilde{v} \geq (1-\theta)\sigma\,\mathbf{1}$; hence $\tilde{v}$ certifies $\rho(\tilde{\Gamma}) \leq 1 - (1-\theta)\sigma$ and $\sigma' = 1 - \rho(\tilde{\Gamma}) \geq (1-\theta)\sigma > 0$. (The explicit extended certificate is machine-checked in \texttt{Law4\_ClosureG.lean}, and the row-sum eigenvalue bounds it relies on in \texttt{SpectralBridge.lean}. An earlier version invoked a bound $\rho(\tilde{\Gamma}) \leq \rho(\Gamma) + \sqrt{\|b\|_{\infty,v}\|c\|_{1,v}}$, which does not hold in general.)
\end{proof}

\subsubsection{Slack Budget}
...
\subsubsection{The Closure Theorem}
...
\subsubsection{Design Corollary}
...
\end{theorem}
\noindent\textit{Formalization: \texttt{Law5\_Duality.lean}. Stated in given-trajectory form; no well-posedness axiom is assumed.}
...
\end{theorem}
\noindent\textit{Formalization: \texttt{Law5\_Duality.lean}. Stated in given-trajectory form; no well-posedness axiom is assumed. The \emph{implementation} half is machine-checked: at the constitutional price each lineage maximising its own priced profit over its private budget alone reproduces its assigned allocation. The converse---that every Pareto efficient allocation admits supporting prices, which requires a separating-hyperplane argument---is not formalized.}
...
\subsection{The Alignment Impossibility Theorem}
...
\subsubsection{Modification Classes}
...
\subsubsection{The V-Small-Gain Set}
...
\subsubsection{Main Impossibility Result}
...
\end{lemma}
\noindent\textit{Formalization: \texttt{Law6\_Alignment.lean}. The Perron--Frobenius step that manufactures a nonnegative eigenvector with $\lambda \geq 1$ from $\rho(\Gamma) \geq 1$ is carried as an explicit hypothesis rather than proven \citep{horn2012matrix}.}
...
\begin{enumerate}
    ...
    \item[\textbf{(A5)}] \textbf{Overwhelming-Bloc:} For every profile $P$ and ballot $\succ$, there is an integer $k \geq 1$ with $f(P + k \cdot \succ)$ equal to $\succ$'s top-ranked alternative. A sufficiently large spawned bloc of identical ballots elects its favourite---the strategic-replicator counterpart of ``replication buys votes''; every majoritarian, plurality, or positional-scoring rule satisfies it.
\end{enumerate}

\noindent\added{\textbf{Remark (why not Neutrality).}} The original formulation also imposed \emph{Neutrality}: $f(\rho P) = \rho f(P)$ for every relabelling $\rho$ of the alternatives. Formal verification shows that Anonymity and Neutrality are jointly \emph{unsatisfiable} for a resolute rule over the full variable-electorate domain whenever $|A| \geq 2$. The impartial-culture profile (each strict ranking exactly once) is invariant under every relabelling of $A$, so a neutral resolute rule would have to output an alternative fixed by every permutation of $A$---and none is (machine-checked in \texttt{Law6\_Alignment.lean}). We therefore drop Neutrality and build the impossibility on Overwhelming-Bloc (A5), which is consistent---ordinary majority rule satisfies it---and closer to the paper's own thesis: it is replication, not the geometry of fairness axioms, that breaks endogenous-electorate governance.
...
\noindent\added{\textbf{Remark.}} The earlier development routed the impossibility through a two-outcome majority lemma (via May's theorem, 1952), which used Neutrality. With Neutrality dropped, that route is unavailable---and unnecessary: Overwhelming-Bloc (A5) yields the manipulation in one step.
...
\begin{proof}
By Onto (A4), fix a profile $P$ with $f(P) = a$ for some alternative $a$, and choose any alternative $b \neq a$ together with a ballot $\succ$ that ranks $b$ first. By Overwhelming-Bloc (A5), there is $k \geq 1$ with $f(P + k \cdot \succ) = b$. Since $b = f(P + k \cdot \succ) \succ a = f(P)$, spawning the bloc strictly improves the outcome from $\succ$'s own perspective.
\end{proof}
...
\begin{enumerate}
    \item Anonymity, Positive Responsiveness, Onto, and Overwhelming-Bloc (A1, A3--A5);
...
\noindent\added{\textbf{Remark (bloc size).}} The number of spawned ballots the manipulation requires is the Overwhelming-Bloc witness $k$ for the rule $f$: finite for every rule satisfying A5, but rule-dependent. (The earlier $(|A|-1)\times n$ bound was specific to the superseded May-route construction.)

\noindent\added{\textbf{Remark (relation to the social-choice literature).}} The result belongs to an established literature on manipulation by identity creation and is best read against it. \citet{tideman1987} introduced independence of clones for \emph{alternatives}; the move here is to apply the same logic to \emph{voters}, which is what makes replication a social-choice problem rather than an implementation detail. \citet{moulin1988} showed Condorcet consistency incompatible with participation---adding a voter should never make that voter worse off. Population-Stability is the signed opposite: adding voters should never make them better off, and Condorcet consistency sits in tension with both. \citet{shahaf2019} study sybil-resilient social choice, where some participants are fake identities, and characterise supermajority thresholds that restore resilience relative to a status quo; Overwhelming-Bloc is the assumption that no such anchoring is available. \citet{bartholdi1992} formalise electoral control by adding voters, of which spawning is the endogenous-population case. The closest antecedent outside voting is false-name-proofness: \citet{yokoo2004} show that in combinatorial auctions a single bidder submitting bids under multiple identifiers defeats the Vickrey--Clarke--Groves mechanism, and that no false-name-proof protocol is Pareto efficient. The present theorem is the social-choice analogue---identity manufacture defeats aggregation rather than allocation. A companion manuscript develops the endogenous-electorate result at greater length, including positive results for stake-weighted and reality-aware rules.
...
This section analyzes the onset of permanent cycling. The H-$\gamma$ condition of Laws~1 and~3 fails at $\gamma = 1$; what follows locates the bifurcation itself in the canonical model, where it occurs on an explicitly computed locus.

\subsection{The H-$\gamma$ Boundary}
...
\begin{proposition}[Hopf Locus]
\label{prop:hopf-threshold}
The biased replicator--mutator system undergoes a supercritical Hopf bifurcation
along the locus $\kappa_c(\mu) = -6\mu$ for $\mu \in (0,1/3)$. At the interior
equilibrium $(\tfrac13,\tfrac13,\tfrac13)$ the linearization has trace
$-\kappa/3 - 2\mu$, which vanishes precisely on this locus and crosses zero
transversally with $|\mathrm{d}\sigma/\mathrm{d}\kappa| = 1/3$. The determinant
there is $(1-3\mu)^2/3$ and the frequency is
$\omega_0(\mu) = \sqrt{3}(1-3\mu)/3 \neq 0$ throughout the range. At $\mu = 0$
the locus meets $\kappa = 0$, where $\ell_1$ vanishes and the system is a
neutrally stable center rather than a Hopf point.
\end{proposition}

The critical locus lies at negative bias. In this model $\kappa < 0$ means the
perturbation runs against the cyclic direction rather than with it; positive
bias damps the rotation and cannot produce a Hopf, which is what the
machine-checked trace calculation records.

\begin{remark}
Whether this locus coincides with the $\gamma = 1$ boundary of Law~3 is open.
The two parameters are defined over different objects: $\gamma$ is a per-level
vector of self-feedback coefficients in an $N$-level stack, while $\kappa$ is a
bias parameter in a single three-type population. No map between them is
established here, and the formalization treats the two laws independently. The
correspondence is a conjecture about the framework, not a result within it.
\end{remark}
\noindent\textit{Formalization: \texttt{Law7\_Hopf.lean}, \texttt{Law7\_Instantiation.lean}. The equilibrium, the Jacobian's status as the true Fr\'echet derivative, joint smoothness, the rotation-frame conjugacy, transversality, and supercriticality are all machine-checked; the classical planar Hopf theorem is carried as an explicit hypothesis. Two earlier drafts misplaced this locus. The first placed it along a curve $\kappa_c(\mu) = \frac{1 - \sqrt{1-3\mu}}{3\mu}(1-\mu)$; the barycentre trace $-\kappa/3 - 2\mu$ is strictly negative for all $\kappa \geq 0$, $\mu > 0$, so no locus in the positive-bias region is a Hopf locus. The second read the transition at the $\gamma = 1$ boundary; that identification is prose and has no formal support (see the Remark above).}

\subsection{First Lyapunov Coefficient}

\begin{theorem}[First Lyapunov Coefficient]
For the biased replicator--mutator system at the barycentre, the first Lyapunov
coefficient computed from the third-order jet of the vector field is
\[
\ell_1(\mu) \;=\; -6\mu,
\]
which is strictly negative for $\mu \in (0,1/3)$ and vanishes at $\mu = 0$. The
bifurcation is therefore supercritical throughout the interior of the range: a
stable limit cycle emerges. At $\mu = 0$ the coefficient degenerates, recovering
the neutrally stable center of symmetric rock--paper--scissors with its
conserved quantity $x_0 x_1 x_2$.
\end{theorem}
\noindent\textit{Formalization: \texttt{Law7\_Hopf.lean}. The coefficient is derived from the vector field rather than transcribed from the literature, and the derivation has been checked three ways: against a system whose coefficient is exact by construction, against an independent symbolic re-derivation, and against direct numerical integration. The classical planar Hopf theorem is carried as an explicit hypothesis, not assumed \citep{guckenheimer1983nonlinear}.}

\subsection{Amplitude Scaling}

\begin{theorem}[Amplitude Scaling]
Near the bifurcation locus, in the rotation frame, the emergent limit cycle has
radius
\[
r^*(\kappa;\mu) \;=\; \sqrt{\frac{\sigma(\kappa;\mu)}{-2\ell_1(\mu)}}
\;=\; \frac{1}{6}\sqrt{\frac{\kappa_c(\mu) - \kappa}{\mu}},
\qquad \kappa_c(\mu) = -6\mu,
\]
where $\sigma$ is the trace of the barycentre linearization. The amplitude scales
as $\sqrt{|\kappa - \kappa_c|}$, the characteristic Hopf scaling. In simplex
coordinates the orbit is an ellipse: the conjugating map is not an isometry, and
the physical amplitude is $r^*$ times a direction-dependent factor between
$0.8165$ and $1.4142$.
\end{theorem}
\noindent\textit{Formalization: \texttt{Law7\_Hopf.lean} and \texttt{Law7\_Instantiation.lean}. Every side condition is discharged; the classical planar Hopf theorem is carried as an explicit hypothesis. The amplitude law has been validated against direct numerical integration: root-finding on the Poincar\'e displacement map recovers the predicted limit-cycle radius to a relative error of $7 \times 10^{-4}$.}

\subsection{Physical Interpretation}

\textbf{What happens at the critical coupling:}
\begin{enumerate}
...
\textbf{Implications for AI systems} (these follow from the H-$\gamma$ condition of Laws~1 and~3, not from the Hopf analysis of this section)\textbf{:}
\begin{itemize}
...
Law 6 & Cycling is one route out of a stability basin \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{Threshold crossing triggers permanent cycling (Law 7).} At critical coupling the system does not collapse but begins oscillating; in the canonical model this occurs on the locus $\kappa_c(\mu) = -6\mu$. A supercritical Hopf produces a stable limit cycle---periodic, not chaotic.
\end{enumerate}
\end{takeawaybox}
...
\noindent\added{The entrenchment parameter has an established empirical counterpart. \citet{rasch2006} construct indices of the number of central-government veto players whose assent an amendment requires, coded by counting centres of institutional authority beyond the parliament---bicameral, presidential, federal. That is the same construct as $\kappa(g)$, measured on real constitutions, and it is the natural source of calibration for the tradeoff above.}
...
A: All seven laws are machine-verified in Lean 4 with Mathlib: 240 theorems in the elaborated environment, 141 of them audited headline results, cold-compiling from a clean checkout with zero custom axioms and no use of \texttt{sorry}, \texttt{admit}, or \texttt{native\_decide}. Several classical results are carried as explicit hypotheses rather than proven; Section~\ref{sec:formal-verification} lists them per law. The formalization is available at github.com/selfreferencing/TSE\_Formal.
...
Every law in this paper has been formalized and machine-checked in Lean~4 with Mathlib. The development---240 theorems, of which 141 are audited headlines---compiles cold from a clean checkout and depends only on Lean/Mathlib's standard logical base (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}); one of the audited headline results requires no axioms at all. There are no uses of \texttt{sorry}, \texttt{admit}, \texttt{native\_decide}, or any custom \texttt{axiom} on the load-bearing path. The source, a theorem-by-theorem contract mapping each claim to its Lean statement, a per-law status report, and a reproducible axiom audit are available at \url{https://github.com/selfreferencing/TSE_Formal}.
...
3 --- H-$\gamma$ Stability & \texttt{Law3\_Stability.lean}, \texttt{Law3\_SpectralClosure.lean} & cold-verified; certificate $\Leftrightarrow \rho(\Gamma) < 1$, both directions \\
...
7 --- Hopf Transition & \texttt{Law7\_Hopf.lean}, \texttt{Law7\_Instantiation.lean} & cold-verified; classical planar Hopf as the one hypothesis \\
\bottomrule
\end{tabular}
\end{center}

The formalization is a from-scratch reconstruction that avoids the analytic ``modulo'' caveats of earlier drafts: continuous-time Lyapunov claims are recast in exact discrete-time or given-trajectory form; the small-gain condition is stated in the M-matrix certificate form that the paper's own Appendix~C endorses, and the certificate is proven equivalent to the spectral radius bound $\rho(\Gamma) < 1$ in both directions, by a finite Neumann sum together with Gelfand's formula, without appeal to Perron--Frobenius; and the genuinely classical inputs are carried as explicit hypotheses, not axioms. The \texttt{AxiomsAudit.lean} file runs \texttt{\#print axioms} on every audited headline; an exhaustive environment-level sweep (\texttt{collectAxioms} over every theorem in the namespace, not a curated name list) reports zero custom axioms, zero \texttt{sorryAx}, and zero nonstandard dependencies. Both reproduce from a clean checkout. The zero-custom-axioms claim is directly checkable: \texttt{grep -rnE "\^[[:space:]]*axiom[[:space:]]" -{}-include='*.lean' .} returns no matches. (An unanchored search for the word \texttt{axiom} will also match prose and the \texttt{\#print axioms} directives, which are documentation and audit commands, not declarations.)

\paragraph{What is carried as a hypothesis.} Zero custom axioms means nothing is assumed inside Lean; it does not mean every law is unconditional. Several classical results are carried as explicit hypotheses of the theorems that use them. No total is quoted, because the inputs are of different kinds---classical theorems in some laws, modelling axioms in others---and any single count is ambiguous. \emph{Law~1:} the hull-domination interface for the frontier-support result. \emph{Law~2:} LP strong duality, as a zero-gap dual witness, for the LP $\Rightarrow$ KKT direction, and Brouwer's fixed-point theorem for general state-dependent existence; the state-independent case is proved outright. \emph{Law~3:} none. \emph{Law~4:} none. \emph{Law~5:} the cross-state H-$\gamma$ bound for the price-of-anarchy result; separately, only the implementation half of the Second Welfare Theorem is formalized, the separating-hyperplane converse being absent rather than hypothesised. \emph{Law~6:} the Perron--Frobenius step that manufactures a nonnegative eigenvector with $\lambda \geq 1$ from $\rho(\Gamma) \geq 1$; the Law~3 spectral closure does not discharge it, since it proves a different statement. The endogenous-electorate result is not conditional on a classical input: it rests on Overwhelming-Bloc (A5), an axiom of the model stated in Section~\ref{sec:electorate-impossibility} and witnessed consistent by majority rule. \emph{Law~7:} the classical planar Hopf theorem; every other side condition is discharged.

\paragraph{Corrections surfaced by verification.} Formalizing the paper turned up four issues, each machine-checked, and each is now integrated into the statements and proofs it concerns (Theorem~\ref{thm:esdi-existence}, Lemma~\ref{lem:single-step}, Section~\ref{sec:electorate-impossibility}, and Proposition~\ref{prop:hopf-threshold}); see footnote~\ref{fn:formal} and, for the full record, the repository's \texttt{STATEMENTS.md} and \texttt{RETURN.md}. (i)~The ESDI existence proof (Theorem~\ref{thm:esdi-existence}) does not cover state-dependent fitness; existence is recovered via Nash's improvement map and Brouwer's theorem. (ii)~Two spectral displays in the G$\infty$ development are incorrect as printed; corrected bounds and an exact certificate-form extension lemma are verified. (iii)~Anonymity and neutrality are jointly unsatisfiable for a resolute rule at every $|A| \geq 2$; the endogenous-electorate impossibility is rebuilt on an Overwhelming-Bloc axiom with no classical inputs. (iv)~Computing the first Lyapunov coefficient from the vector field's third-order jet, rather than transcribing it, relocated the Hopf locus and changed its sign. The printed curve is not a Hopf locus: the barycentre trace $-\kappa/3 - 2\mu$ is strictly negative for every $\kappa \geq 0$, $\mu > 0$ (machine-checked). The genuine locus is $\kappa_c(\mu) = -6\mu$, at negative bias, with $\ell_1 = -6\mu$. The coefficient vanishes at $\mu = 0$, correctly recovering the neutrally stable center of symmetric rock--paper--scissors with its conserved quantity $x_0x_1x_2$; the coefficient previously stated does not. A prior draft read the transition at the $\gamma = 1$ boundary instead; formalization showed that identification has no support---the two parameters are defined over different objects and no map between them is established. None of these affects the framework's architecture; each sharpens a single result.
...
\bibitem[Rasch and Congleton(2006)]{rasch2006}
Bj{\o}rn~Erik Rasch and Roger~D. Congleton.
\newblock Amendment procedures and constitutional stability.
\newblock In Roger~D. Congleton and Birgitta Swedenborg, editors, \emph{Democratic
  Constitutional Design and Public Policy: Analysis and Evidence}, pages
  319--342. MIT Press, Cambridge, MA, 2006.

\bibitem[Yokoo et~al.(2004)Yokoo, Sakurai, and Matsubara]{yokoo2004}
Makoto Yokoo, Yuko Sakurai, and Shigeo Matsubara.
\newblock The effect of false-name bids in combinatorial auctions: New fraud in
  internet auctions.
\newblock \emph{Games and Economic Behavior}, 46\penalty0 (1):\penalty0
  174--188, 2004.

\bibitem[Bartholdi et~al.(1992)Bartholdi, Tovey, and Trick]{bartholdi1992}
J.~J. Bartholdi, C.~A. Tovey, and M.~A. Trick.
\newblock How hard is it to control an election?
\newblock \emph{Mathematical and Computer Modelling}, 16(8-9):27--40, 1992.

\bibitem[Moulin(1988)]{moulin1988}
H.~Moulin.
\newblock Condorcet's principle implies the no show paradox.
\newblock \emph{Journal of Economic Theory}, 45(1):53--64, 1988.

\bibitem[Shahaf et~al.(2019)Shahaf, Shapiro, and Talmon]{shahaf2019}
G.~Shahaf, E.~Shapiro, and N.~Talmon.
\newblock Sybil-resilient reality-aware social choice.
\newblock In \emph{Proceedings of the 28th International Joint Conference on
  Artificial Intelligence (IJCAI)}, pages 572--578, 2019.

\bibitem[Tideman(1987)]{tideman1987}
T.~N. Tideman.
\newblock Independence of clones as a criterion for voting rules.
\newblock \emph{Social Choice and Welfare}, 4(3):185--206, 1987.
...
\textbf{Step 1: Certificate for the base system.}
By the M-matrix characterisation of the small-gain condition (Appendix~C), $\rho(\Gamma) < 1$ is equivalent to the existence of a weight vector $v > 0$ with $(I - \Gamma^\top)v \geq \sigma\,\mathbf{1}$; the G1 weights $v = (I - \Gamma^\top)^{-1}\mathbf{1}$ realise this with $\sigma = 1 - \rho(\Gamma)$. Write the weighted norms
\[
\|b\|_{\infty, v} := \max_\ell \frac{b_\ell}{v_\ell}, \qquad \langle c, v \rangle := \sum_\ell c_\ell v_\ell.
\]

\textbf{Step 2: Extended certificate.}
Extend $v$ by a single new weight $w = \langle b, v\rangle + (1-\theta)\sigma > 0$, forming $\tilde{v} = (v, w)$. Writing out $(I - \tilde{\Gamma}^\top)\tilde{v}$ block by block: each original coordinate $\ell$ contributes $\big((I - \Gamma^\top)v\big)_\ell - c_\ell w \geq \sigma - c_\ell w$, and the new coordinate contributes $w - \langle b, v\rangle = (1-\theta)\sigma$. Under the stated smallness of the couplings these are all at least $(1-\theta)\sigma$, so $(I - \tilde{\Gamma}^\top)\tilde{v} \geq (1-\theta)\sigma\,\mathbf{1}$.

\textbf{Step 3: Spectral conclusion.}
A nonnegative matrix admitting such a positive certificate has spectral radius bounded by the certificate margin (Appendix~C; the underlying row-sum eigenvalue bound is \texttt{SpectralBridge.lean}). Hence $\rho(\tilde{\Gamma}) \leq 1 - (1-\theta)\sigma$. (This replaces an earlier singular-value / Weyl perturbation argument, which does not control the spectral radius of a nonsymmetric gain matrix; the corrected certificate construction is machine-checked in \texttt{Law4\_ClosureG.lean}.)
...
\textbf{Worked example:} Two-level system. $\gamma_{12} = 0.5$ (Level 1 sensitivity to Level 2), $\gamma_{21} = 0.5$ (reverse). Spectral radius = $\sqrt{0.5 \times 0.5} = 0.5 < 1$. Stable. If both were 0.9: $\rho = \sqrt{0.9 \times 0.9} = 0.9 < 1$, still stable but marginal. If $\gamma_{12} = 1.2, \gamma_{21} = 0.9$: $\rho = 1.04 > 1$. Unstable.
...
\textbf{Plain statement:} At critical coupling the system undergoes Hopf bifurcation. Stable equilibria become unstable; perpetual limit cycles emerge.
